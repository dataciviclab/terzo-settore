"""Motore matching ETS-bando: score, filtro geografico, scan completo.

Migrato da radar/core.py. La logica è identica — solo import aggiornati.
"""
...
TERRITORY_KEYWORDS: dict[str, list[str]] = {
...
def parse_date_flex(s: str) -> tuple[date | None, int]:
    oggi = date.today()
    if not s:
        return None, 999
    s = s.strip().lower()
...
def fmt_match_reason(c: dict) -> str:
...
def extract_territory(b: dict) -> list[str]:
...
def classify_bando(b: dict, scadenza: date | None, gg_rimasti: int) -> tuple[str, str]:
...
def load_bandi(files: list[Path] | None = None) -> list[dict]:
    bandi = []
...
def normalise_bando(b: dict) -> tuple:
...
def is_sport_bando(tags: list[str]) -> bool:
    return any((t or "").strip().lower() == "sport" for t in tags)


# ── Core matching ─────────────────────────────────────────────────────

def match_bando(con, pattern: str, tags: list[str], limit: int = 10,
...
def run_scan(con=None, bandi=None, match_limit=10, include_statuses=None):
    from tsi.lib.html_utils import arricchisci
