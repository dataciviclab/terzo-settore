...
La logica e' adattata da cruscotto-italia/etl/sources/runts.py (schema XLSX
verificato) e resa self-contained per TSI.
...
def download_xlsx() -> bytes:
    ...
    if "octet-stream" not in ct and "spreadsheet" not in ct:
        raise RuntimeError(
            f"Risposta non XLSX: content-type={ct} (size={len(r.content)} bytes). "
            "Probabilmente la sessione e' scaduta o il bottone e' cambiato."
        )

    elapsed = time.time() - t0
    print(f"runts_download_ok bytes={len(r.content)} elapsed_s={round(elapsed, 1)}")
    return r.content
